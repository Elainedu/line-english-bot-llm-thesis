    ...
    def _calculate_similarity(self, original_text, recognized_text):
        ...
        try:
            ...
            try:
                ...
                # 使用新的評分公式 - 指數衰減轉換
                # 設定縮放因子，值越小對距離變化越敏感
                scaling_factor = max_len / 4  # 根據文本長度自動調整縮放因子
                
                if max_len > 0:
                    # 使用指數衰減公式計算相似度
                    similarity = 100 * math.exp(-distance / scaling_factor)
                else:
                    similarity = 0
                    
                return similarity  # 返回百分比相似度
    ...
    def _generate_feedback(self, user_id, original_text, recognized_text, similarity_score, difficulty, attempts):
        ...
        try:
            ...
            if similarity_score >= 85:
                ...
                score_1_to_5 = 1  # 極差
                performance = "需要大幅改進"
                feedback = "建議多聽幾次原音，注意每個單詞的發音，慢慢跟讀後再逐漸加快速度。"
            
            # 保存評估結果，包括1-5分評級
            user_session = UserSession.get_by_line_user_id(user_id)
            if user_session:
                user_session.update({"last_score": similarity_score, "recognized_text": recognized_text, "star_rating": score_1_to_5})
            
            # 準備反饋消息
            messages = []
            
            # 主要評估結果
            result_message = f"評估結果：{performance} ({score_1_to_5}/5 分)\n"
    ...
    def _generate_mock_evaluation(self, user_id, original_text, difficulty, attempts):
        ...
        try:
            ...
            if similarity_score >= 85:
                score_1_to_5 = 5  # 優秀
                performance = "優秀"
                feedback = "模擬評估：你的跟讀表現非常好！"
            elif similarity_score >= 70:
                score_1_to_5 = 4  # 良好
                performance = "良好"
                feedback = "模擬評估：你的跟讀表現良好，繼續努力！"
            elif similarity_score >= 50:
                score_1_to_5 = 3  # 中等
                performance = "中等"
                feedback = "模擬評估：你的跟讀表現中等，可以繼續提高。"
            elif similarity_score >= 30:
                score_1_to_5 = 2  # 較差
                performance = "需要改進"
                feedback = "模擬評估：繼續練習，你會進步的！"
            else:
                score_1_to_5 = 1  # 極差
                performance = "需要大幅改進"
                feedback = "模擬評估：多聽幾次再嘗試跟讀，慢慢來！"
            ...
            if user_session:
                user_session.update({
                    ...
                    "recognized_text": "無法使用語音識別",
                    "star_rating": score_1_to_5
                })
            ...
            result_message += f"表現等級：{performance} ({score_1_to_5}/5 分)\n"
            result_message += f"相似度得分：{similarity_score:.1f}%\n\n"
            result_message += f"原始句子：\n{original_text}\n\n"
            result_message += f"反饋：{feedback}\n\n"
